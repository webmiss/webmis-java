...
import vip.webmis.mvc.util.Base64;
...
public class Upload {
  ...
  public static String File(MultipartFile file, HashMap<String, Object> params) {
    ...
    if(param.get("bind") != null) {
      if(!bind.contains(ext)){
        System.out.println("只支持" + Util.Implode(",", bind.toArray(new String[0])) + "格式!");
        return "";
      }
    }
    // 是否重命名
  ...
  public static String Base64(HashMap<String, Object> params) {
    ...
    // 否有类型
    String[] ct = Util.Explode(",", base64);
    if (ct.length > 1) {
      param.put("ext", Base64.GetExt(ct[0]));
      base64 = ct[1];
    }
    // 创建目录
    FileEo.Root = Env.root_dir;
  ...
  static public String GetFileName() {
    ...
    return df.format(now) + randA + randB;
  }
